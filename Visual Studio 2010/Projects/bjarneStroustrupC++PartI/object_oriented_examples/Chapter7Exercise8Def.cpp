/*
	TITLE			   Grammar and factorial	Chapter7exercise8Def.cpp
...
Token Token_stream::get()
{
	...
	switch (ch)
	{
		...
		case '*': case '/': case '%': case '=': case '!': // New code
		{
			return Token(ch);
		}
		case '.':
...
{
	std::cout << "\nCalculator 6.0\t Author: Chris B. Kirov\t Date: 26.02.2015\n";
	std::cout << "\nSupported operations (in decreasing order of precedence):\n";
	std::cout << "1.'('Expression')'; pow(base,exp); sqrt(Expresion)\n";
...
double primary()
{
	...
	switch (t.kind) 
	{
		...
		case numberIdentifier: // New code
		{	
			Token t2 = ts.get();
