/*
	TITLE			   Starlike patterns 	   Chapter12Exercise13.cpp
...
void SuperEllipse::drawStar()
{
	...
	Simple_window sw(Point((x_max()- swWidth) / 2. , (y_max() - swHeight) / 2.), 
			         swWidth, swHeight, "Chapter 12 Exercise 13");
	...
	for(size_t i = coordinates.size() / 100; i < coordinates.size(); ++i)
	{
			for(size_t j = i + 1; j < coordinates.size(); ++j)
			{
				...
				// set one of two colors to each line
				generatedLines[generatedLines.size() - 1]->set_color((j % 2 == 0) ? Color::blue : Color::red);
...
void SuperEllipse::drawSuperEllipse()
{
	...
	{ 
		...
		Graph_lib::Line* superEllipse = new Line(Point(coordinates[i]), Point(coordinates[i + 1]));

		// set color
		superEllipse->set_color((i % 2 == 0) ? Color::red : Color::blue);

		// store supperellipse (to free in destructor)
		generatedLines.push_back(superEllipse);
