...
{
    // matrixes store their memory (elements) in Matrix_base and have copy semantics
...
A apply (F f, A x)
{ 
	A res(x, f);  // construct a new Matrix, res, with element's that are functions of x's elements  
	return xfer(res); // let 'res' own its resources
}
